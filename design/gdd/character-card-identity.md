...
| 14 | ~~**Durability timing của Entity Record mới tạo**~~ — **ĐÃ ĐÓNG
  2026-08-12, ADR-0004** (`docs/architecture/adr-0004-turn-manager-undo.md`,
  cũng là QQ-03 của `architecture.md`): sự kiện `card_exists` chuyển `true`
  (Entity Record TẠO MỚI) nay đi kèm ghi `turn_records` MỖI LƯỢT — cùng
  boundary bền vững với `world_time` — thay vì chỉ theo chu kỳ 50-lượt của
  `fixed_blob_bytes`. Nội dung field CẬP NHẬT (không phải tạo mới) của
  Entity Record vẫn theo chu kỳ 50-lượt như cũ (rủi ro còn lại: hẹp, chỉ
  mất vài field cập nhật gần nhất khi crash, KHÔNG mất sự tồn tại của thẻ) | ~~Persistence + hệ này~~ — đã đóng | — |
