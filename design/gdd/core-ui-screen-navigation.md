...
> **Last Updated**: 2026-09-02 — UX correction to the 2026-09-01 composer amendment (Core Rule #3c, UI Requirements "Composer hành động có cấu trúc", AC-81), per chủ dự án phản hồi trực tiếp khi kiểm thử bản build đầu tiên: "Tường thuật"/"Đối thoại" là 2 nút THÊM đoạn độc lập (mỗi lần bấm tạo ngay 1 khối mới, sửa trực tiếp tại chỗ), KHÔNG phải toggle chọn loại cho 1 đoạn nháp dùng chung như bản 2026-09-01 ban đầu triển khai sai — khái niệm "đoạn nháp"/nút "+ Thêm đoạn" đã bị gỡ bỏ hoàn toàn. Formula D.7 (data schema, validation, resolve_speaker) không đổi — chỉ tầng UI/tương tác được sửa. Bản 2026-09-01 gốc: additive amendment (Core Rule #3c, Formula D.7, AC-71–82, nhóm F) thay ô tự do bằng composer hành động có cấu trúc (Tường thuật/Đối thoại NV chính/Đối thoại NPC, xen kẽ tự do, tìm kiếm NPC đã biết + gõ tên NPC mới) theo quyết định chủ dự án — no re-review of THIS GDD's own approval was triggered; xem thêm 2026-08-07 additive edits từ `/design-review persistence-save-system.md` round 3 (new `O-ConfirmDelete` overlay + AC-64/AC-70/D.2 graph entries, `slot_closure_reason` display split, new-device empty-state note, export description sync) bên dưới; see `design/gdd/reviews/core-ui-screen-navigation-review-log.md` for the 6-round history this builds on.
...
- Composer sống trong **đúng subtree bị khóa đệ quy** khi Resolving/Undoing (Godot notes, mở rộng nguyên trạng từ "ô tự do" cũ) — mọi thao tác soạn thảo bên trong (2 nút thêm đoạn, sửa nội dung 1 khối, xóa 1 khối, gõ ô tìm NPC) bị khóa **cùng lúc** với nút Gửi, không tách riêng thành 1 lớp gate mới. **SỬA 2026-09-02**: mỗi khối, một khi đã thêm, giữ nguyên loại (Tường thuật hoặc Đối thoại) suốt vòng đời của nó — không có thao tác "đổi loại 1 khối đã có" (2 nút "Tường thuật"/"Đối thoại" luôn THÊM khối mới, không bao giờ sửa khối cũ).
...
- **Bấm liên tiếp cùng 1 nút thêm đoạn** (VD "Đối thoại" 3 lần liên tục): mỗi lần thêm đúng 1 khối Đối thoại rỗng mới vào cuối danh sách — không giới hạn số khối cùng loại, không gộp/thay thế khối trước đó (SỬA 2026-09-02 — thay cho edge case "đổi loại đoạn" cũ, không còn áp dụng vì không có thao tác đổi loại 1 khối đã tồn tại).
...
*(Bổ sung 2026-09-01, thay thế hoàn toàn ô tự do đơn dòng cũ — xem Core Rule #3c, D.7. **SỬA 2026-09-02** theo phản hồi trực tiếp của chủ dự án khi kiểm thử bản build đầu tiên: "Tường thuật"/"Đối thoại" KHÔNG phải toggle chọn loại cho 1 đoạn nháp dùng chung — đây là 2 NÚT THÊM ĐOẠN độc lập, mỗi lần bấm tạo ngay 1 khối mới, sửa trực tiếp tại chỗ, không qua bước "nháp rồi chốt". Toàn bộ mục này viết lại theo đúng mô hình đã sửa; KHÔNG còn khái niệm "đoạn nháp" hay nút "+ Thêm đoạn".)*

- **Vị trí**: đúng chỗ "ô tự do + nút gửi" cũ trong thứ tự khu nhập (4 thẻ gợi ý → chip intent → **composer** → nút gửi + dòng nudge).
- **2 nút thêm đoạn, luôn hiện, không phải toggle**: "Tường thuật" và "Đối thoại" là 2 nút hành động độc lập (≥44px mỗi ô), không mang trạng thái "đang chọn/chưa chọn" — mỗi lần bấm **THÊM NGAY 1 khối đoạn mới, rỗng, có thể sửa trực tiếp** vào cuối danh sách, không hề đổi hay ghi đè bất kỳ khối nào đã có. Bấm "Tường thuật" rồi bấm "Đối thoại" → có ngay 2 khối trên màn hình, đúng thứ tự đã bấm, mỗi khối sửa độc lập.
- **Mỗi khối trong danh sách là 1 vùng nhập LIVE** (không phải bản đọc lại tĩnh của một đoạn đã "chốt" — không còn khái niệm chốt/nháp):
  - Khối **Tường thuật**: nhãn nhỏ "Tường thuật" (font-weight 300, không phải tap-target) + 1 textarea auto-grow sửa trực tiếp nội dung đoạn đó.
  - Khối **Đối thoại**: nhãn nhỏ "Đối thoại" + hàng chọn người nói (nút "Nhân vật chính" + ô tìm/gõ tên NPC, mặc định Nhân vật chính) + 1 textarea auto-grow cho nội dung câu nói — CẢ HAI đều sửa trực tiếp trong chính khối đó.
  - Mỗi khối có nút xóa riêng (✕, ≥`TOUCH_TARGET_MIN`, D.4a) — xóa được BẤT KỲ khối nào, không chỉ khối cuối; các khối còn lại giữ nguyên thứ tự tương đối.
- **Ô tìm NPC**: chỉ hiện bên trong khối Đối thoại khi người nói≠Nhân vật chính; gợi ý realtime cục bộ theo ĐÚNG khối đó (không ảnh hưởng khối khác), không gọi mạng, không tốn lượt; mỗi gợi ý = `display_name`; luôn có dòng cuối tường minh "Dùng tên mới: '…'".
- **Nút Gửi**: cuối composer; disabled khi `composer_payload_submit_allowed=0` (D.7) — kiểm tra trực tiếp trên danh sách khối hiện có, không có bước "gộp đoạn nháp" nào nữa vì mọi khối đều đã là dữ liệu thật.
- **Cuộn nội bộ**: danh sách khối tự cuộn khi vượt chiều cao khả dụng của khu nhập — không đẩy khung tường thuật cuộn theo; khối đang sửa có thể tạm khuất khỏi vùng nhìn khi danh sách dài (dữ liệu KHÔNG mất, chỉ cần cuộn lại đúng khối đó).
- Tap + click parity toàn bộ; mọi phần tử độc lập (2 nút thêm đoạn, nút xóa từng khối, gợi ý NPC, Gửi) ≥ `TOUCH_TARGET_MIN` (D.4b).
...
- **AC-79** [Integration] (khóa composer khi Resolving/Undoing — mở rộng AC-05): GIVEN `tm_state=resolving`, WHEN thử bấm nút thêm đoạn, sửa nội dung 1 khối, xóa 1 khối, hoặc gõ ô tìm NPC, THEN mọi thao tác bị nuốt ở tầng UI — verify cây node composer nằm trong đúng subtree khóa đệ quy như "ô tự do" cũ.
...
- **AC-81** [Unit] (nút thêm đoạn luôn tạo khối mới, không sửa khối đã có — SỬA 2026-09-02, thay cho AC-81 cũ về "đổi loại giữa chừng" không còn áp dụng): GIVEN danh sách đã có 1 khối `{type: narration, text: "Nàng bước tới"}`, WHEN bấm nút "Đối thoại", THEN danh sách có đúng 2 khối — khối 1 giữ nguyên y hệt (`type: narration, text: "Nàng bước tới"`, không bị đổi/xóa), khối 2 mới là `{type: dialogue, text: "", speaker: {player}}` được thêm vào CUỐI danh sách.
